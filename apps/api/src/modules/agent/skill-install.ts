...
// - /agent/skill/gb-crm/install.sh：安装器（shell），探测 AGENT 技能目录、下载 skill 文件、
//   最后引导用用户名/密码授权（复用 /agent/login.sh → PAT → ~/.gb-crm/credentials.json）。
...
const INSTALL_TEMPLATE_PATH = fileURLToPath(new URL("./install.sh", import.meta.url));
...
export function renderSkillInstallScript(baseUrl: string): string {
  ...
  const template = fs.readFileSync(INSTALL_TEMPLATE_PATH, "utf8");
