...
import type { S3ConfigDto } from "../api/types";
...
export function StorageTab() {
  ...
  const { data: config } = useQuery({
    queryKey: ["system", "s3-config"],
    queryFn: async () => (await api.get<{ data: S3ConfigDto }>("/system/s3-config"))?.data,
  });
  ...
  const saveConfig = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    try {
      await api.patch("/system/s3-config", buildBody());
      setForm((f) => ({ ...f, secretAccessKey: "" }));
      await queryClient.invalidateQueries({ queryKey: ["system", "s3-config"] });
      showToast("已保存远程备份配置");
    } catch (err) {
  ...
  const testConnection = async () => {
    setTesting(true);
    try {
      const res = await api.post<{ data: { probeKey: string } }>("/system/s3-config/test");
        ...
        <div className="card-body">
          ...
          <form className="settings-form" onSubmit={(e) => void saveConfig(e)}>
            ...
            <label className="field">
              ...
              <input
                autoComplete="off"
                placeholder="如 https://s3.example.com（MinIO / OSS / COS / R2 均可）"
                value={form.endpoint}
            ...
            <label className="field">
              ...
              <input
                autoComplete="off"
                placeholder="可空；如 backups/（自动归一化结尾斜杠）"
                value={form.prefix}
