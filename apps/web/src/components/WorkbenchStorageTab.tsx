// 工作台分发 tab（K61，系统设置页）：S3 兼容对象存储配置（system_configs code='workbenchS3'，仅 admin）。
// 保存并启用后，gb-content 系统层快照发布到该桶（内容寻址 objects/<sha256>）。secret 只回掩码：留空 = 不改。
import { useEffect, useState, type FormEvent } from "react";
import { useQuery, useQueryClient } from "@tanstack/react-query";

import { api, ApiError } from "../api/client";
import type { WorkbenchS3ConfigDto } from "../api/types";
...
function formFromDto(cfg: WorkbenchS3ConfigDto): StorageForm {
  return {
    enabled: cfg.enabled,
...
export function WorkbenchStorageTab() {
  const showToast = useToast();
  const queryClient = useQueryClient();
  const { data: config } = useQuery({
    queryKey: ["system", "workbench-s3-config"],
    queryFn: async () =>
      (await api.get<{ data: WorkbenchS3ConfigDto }>("/system/workbench-s3-config"))?.data,
  });
  ...
  const saveConfig = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    try {
      await api.patch("/system/workbench-s3-config", buildBody());
      setForm((f) => ({ ...f, secretAccessKey: "" }));
      await queryClient.invalidateQueries({ queryKey: ["system", "workbench-s3-config"] });
      showToast("已保存工作台分发配置");
    } catch (err) {
  ...
  const testConnection = async () => {
    setTesting(true);
    try {
      const res = await api.post<{ data: { probeKey: string } }>("/system/workbench-s3-config/test");
    ...
    <div className="settings-section">
      <div className="card">
        <div className="card-head">
          <h2>工作台分发（S3 兼容对象存储）</h2>
        </div>
        <div className="card-body">
          <p style={{ marginTop: 0, fontSize: 13 }}>
            启用后，gb-content 工作台的系统层快照会发布到该桶（与「远程备份」「资料存储」互相独立，可填同一服务商不同
            Bucket / 前缀）。团队成员用同一 CRM 账号经 HTTP 增量同步，无需接触 git。
          </p>
          <form className="settings-form" onSubmit={(e) => void saveConfig(e)}>
            ...
            <label className="field">
              ...
              <input
                autoComplete="off"
                placeholder="如 gb-content-workbench"
                value={form.bucket}
            ...
            <label className="field">
              ...
              <input
                autoComplete="off"
                placeholder="可空；如 workbench/（自动归一化结尾斜杠）"
                value={form.prefix}
