...
- `workbench`（K61）工作台快照分发（gb-content → 成员机，授权复用 K35 PAT）：发布仅 admin——multipart `git archive` tar.gz → 零依赖手写 tar 读取器 `tar.ts`（ustar/pax、拒链接与 zip-slip/控制字符、条目/总量/路径三重上限）→ 文件按内容寻址存 workbenchS3 桶 `{prefix}workbench/objects/<sha256>`（先 ListObjects 去重只 PUT 新对象）→ `workbench_versions` 不可变版本行（manifest_json，滚动留 10 份硬删 + 孤儿对象尽力 GC）；读取任意已认证身份（read PAT 即可）：`GET /workbench/versions(/latest/:id)`、`manifest.tsv`（脚本友好 TSV）、`objects/:sha`（CRM 代理 S3 回包，ETag=sha256）；安装器 `GET /agent/workbench/install.sh` + `/agent/workbench/install.ps1`（Windows PowerShell 版，纯 ASCII 模板，同 skill 安装器双脚本模式）公开下发（凭证复用 `~/.gb-crm/credentials.json`，装到 `~/闪光团队工作台`）；客户端 sync/publish 脚本在 gb-content 仓库；前端系统设置 tab「工作台分发」（admin）
