...
- 路由：`/login` `/my/customers` `/my/deals` `/customers` `/customers/:id`（总览）`/channels` `/products` `/deals` `/deliveries` `/deliveries/:id` `/deliveries/:id/circle` `/deliveries/:id/gantt` `/deliveries/:id/matrix` `/delivery-types` `/materials`（资料专区）`/materials/:id/edit`（资料全文编辑页，仅文本类）`/users` `/settings`（系统设置，tab：LLM 打标配置 admin + 角色权限 admin + 远程备份 admin + 后台任务全角色 + 定时任务 admin）`/tokens`（授权管理，仅 admin：列全部 Agent PAT 令牌 + 吊销任意，见 K35 治理）`/business-settings`（业务设置，客户标签词表）（默认进客户）
