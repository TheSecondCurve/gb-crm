...
- PATCH 内核：JSON **键存在** → SET（`null` 清空可空列）；**键缺席** → 不动。关系数组同理：缺席不动，`[]` 清空。客户归属人 `ownerId` 单值（K39）：缺席不动、`null` 清空。行级 OCC 用 `updatedAt`；客户端每行一条队列、串行、每次带上一次 200 的 `updatedAt`。
...
四张主数据的权限化 CRUD，Excel 式就地编辑。运营流程（成交 / 跟进 / 权益 / 活动）是 Phase 2，不要在 v1 加。
...
5. **客户信息 `/customers`**：分页、模糊搜索、标签、来源渠道、归属人（单值 `owner_id`，K39）；预留可空唯一 `wechat_openid`（不接小程序）。导出 Excel：`GET /api/v1/customers/export.xlsx`（exceljs 服务端生成，复用列表同一 WHERE，跟随 q/类型筛选，不分页）。
