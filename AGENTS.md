...
- 路由：`/login` `/my/customers` `/my/deals` `/customers` `/customers/:id`（总览）`/channels` `/products` `/deals` `/deals/commissions`（成交分成，K56）`/deliveries` `/deliveries/:id` `/deliveries/:id/circle` `/deliveries/:id/gantt` `/deliveries/:id/matrix` `/delivery-types` `/materials`（资料专区）`/materials/:id/edit`（资料全文编辑页，仅文本类）`/users` `/settings`（系统设置，tab：LLM 打标配置 admin + 角色权限 admin + 远程备份 admin + 资料存储 admin + 后台任务全角色 + 定时任务 admin）`/tokens`（授权管理，仅 admin：列全部 Agent PAT 令牌 + 吊销任意，见 K35 治理）`/business-settings`（业务设置，客户/资料标签词表）（默认进客户）
...
6. **成交记录 `/deals`**（K42）：客户（单值 FK 必填）、意向产品、负责人（单值 FK 可空）、阶段（赠送/已付款/退款/已关闭）、订单号、成交日期（`deal_date` 非空、新建必填）、交付日期（`delivery_date` 可空）、支付信息备注；客户城市只读列。assistant 只读。
