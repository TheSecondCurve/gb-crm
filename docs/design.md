...
| K60 | **文案工作台（私域运营分组）**：新侧栏分组「私域运营」+ 菜单页「文案工作台」`/copywriting`，按私域内容方法论用 LLM 生成与审计文案。① 两张新表（`0030_copywriting.sql`）：`copy_templates` 提示词模板词表（`dimension` 六值 background 业务背景/audience 目标客群/topic 主题内容/goal 预期目的/outputType 产出类型/polish 润色要求 + name + content + sort/enabled + 审计四列 + 软删；live 唯一 `(dimension,name) WHERE deleted_at IS NULL`，同域同名 409，仿 tags K45）；`copy_items` 已保存文案（title + **六段维度文本快照** + content + `audit_report` 审计 JSON 快照 + 审计四列 + 软删）。② **模板与文本解耦**：「选模板」= 前端把模板 content 填入输入框可再编辑（本次自定义），generate/audit 只收最终文本快照、不解析模板 id——历史文案不受模板改动影响。③ API（新模块 `modules/copywriting` 三层，新 ACL 资源 `copywriting`：admin/operator 全量、assistant 只读；模板维护 operator 可写——提示词模板是运营内容资产，不同于 tags 词表 admin-only；generate/audit 挂 create 动作）：`GET/POST /copywriting/templates`、`PATCH/DELETE :id`（OCC）；`POST /copywriting/generate`（topic 必填，其余五段可空；prompt 六段按中文标签拼接空段省略，temperature 0.7，输出 `{"content"}`）→ `{data:{content}}`；`POST /copywriting/audit`（用户视角审计：是否达到预期目的/是否有不适表达/是否有不清晰信息提高理解成本，temperature 0，输出 `{verdict: pass|warn|fail, summary, issues:[{aspect,detail,suggestion}]}`，宽松解析逐字段兜底 verdict 非法→warn）；`GET/POST /copywriting/items`、`GET/PATCH/DELETE :id`（q LIKE title/content + OCC + 软删）。④ LLM 复用 `system_configs` code='llm'（`getAiConfig`），未配置 422、失败 502 `LLM_ERROR`，测试 `llmFetch` 注入 mock（同 K46）。⑤ 前端 `/copywriting` 三 tab：生成与审计（六维度 = 模板下拉 + 可编辑 textarea；生成 → 结果卡片 + 复制 + AI 审计卡片 + 保存 Modal 带六段快照与审计快照）、已保存文案（q + 分页 + 查看/编辑/删除，assistant 只读）、模板管理（六维度卡片词表 CRUD，无写权限只读）。**不做**：模板版本历史、文案与客户/交付关联、FTS（q LIKE 即可）、后台任务化（同步请求）、流式输出 | 2026-09-11 产品决策（用户拍板：六维度方法论 + 模板可选可自定义 + 用户视角审计 + 保存备查）。`0030_copywriting.sql` 迁移 |
