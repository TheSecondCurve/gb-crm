...
/** 侧栏菜单分组顺序（渲染与排序用） */
export const PAGE_GROUPS = ["我的运营", "主数据", "运营流程", "私域运营", "客户洞察", "系统", "业务设置"] as const;
...
export const PAGE_REGISTRY: readonly PageDef[] = [
  ...
  { key: "deal-payout-batches", path: "/deals/payout-batches", label: "分成发放", group: "主数据", order: 6, menu: true, required: { resource: "dealCommissions", action: "list" } },
  { key: "deal-payout-batch-detail", path: "/deals/payout-batches/:id", label: "批次详情", group: "主数据", order: 7, menu: false, parent: "deal-payout-batches", required: { resource: "dealCommissions", action: "read" } },
  // ── 运营流程 ──
  ...
  { key: "insights-match", path: "/insights/match", label: "缘分清单", group: "客户洞察", order: 5, menu: true, required: { resource: "insights", action: "list" } },
  { key: "insights-topics", path: "/insights/topics", label: "洞察词表", group: "客户洞察", order: 6, menu: true, required: { resource: "insights", action: "list" } },
  // ── 系统 ──
